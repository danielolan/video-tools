...
const VideoListItem: React.FC<VideoListItemProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[auto,minmax(0,1fr),100px,100px,200px] gap-4 items-center p-4 border-b border-gray-200 hover:bg-custom-gray">
    <div className="pl-4">
      ...
      <div className="text-right md:block hidden">{videosCount}</div>
      <div className="text-right md:block hidden">{size}</div>
      <div className="text-right pr-8 md:block hidden">{lastModified}</div>
    </div>
  );
};
